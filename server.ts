...
import { authenticateToken, requireAdmin } from './src/server/auth.js';
import {
...
async function startServer() {
  ...
  // Books Catalog
  app.get('/api/books', authenticateToken as any, getBooks as any);
  app.get('/api/books/:id', authenticateToken as any, getBookDetails as any);
  ...
  // Authors Management
  app.get('/api/authors', authenticateToken as any, getAuthors as any);
  ...
  app.get('/api/categories', authenticateToken as any, getCategories as any);
  ...
  app.get('/api/publishers', authenticateToken as any, getPublishers as any);
  ...
  // New Borrow & Return Custom Spec Endpoints
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[Server] Library Management System active at http://localhost:${PORT}`);
  });
}
